...
# 语言字符串表：每个用户可见的图表/控制台字符串都在此注册一个键，
# 覆盖热力图/散点图标题、坐标轴标签、统计面板文字以及控制台提示信息。
_LANG = {
    "zh": {
        ...
        "court_width": "场地宽度 (米)",
        "court_length": "场地长度 (米)",
        "heatmap_saved": "热力图已保存至",
        "scatter_saved": "散点图已保存至",
        "rally_stats_header": "回合 {rally_id} 统计:",
        "match_stats_header": "比赛统计",
        "upper_player_label": "上场球员:",
        "lower_player_label": "下场球员:",
        "avg_speed_label": "平均速度",
        "max_speed_label": "最大速度",
        "distance_moved_label": "移动距离",
        "total_distance_label": "总移动距离",
        "avg_distance_per_rally_label": "平均每回合距离",
        "speed_unit": "米/秒",
        "distance_unit": "米",
        "upper_rally_legend": "上场球员 回合 {rally}",
        "upper_players_legend": "上场球员",
        "lower_rally_legend": "下场球员 回合 {rally}",
        "lower_players_legend": "下场球员",
        "viz_error": "可视化过程中出错",
        "analyzing_data": "分析球员位置数据",
        "analysis_complete": "球员位置分析完成，可视化结果已保存至",
        "analysis_failed": "球员位置分析失败",
        "select_file_prompt": "请选择 detections.jsonl 文件...",
        "file_dialog_title": "选择球员位置检测文件",
        "jsonl_filetype_label": "JSONL文件",
        "all_files_label": "所有文件",
        "no_file_selected": "未选择文件，退出程序",
        "test_success": "可视化测试成功完成",
        "test_failed": "可视化测试失败",
        "test_error": "测试错误",
    },
    "en": {
        "heatmap_title": "Player Position Heatmap",
        "scatter_title": "Player Position Scatter Plot",
        "court_width": "Court Width (meters)",
        "court_length": "Court Length (meters)",
        "heatmap_saved": "Heatmap saved to",
        "scatter_saved": "Scatter plot saved to",
        "rally_stats_header": "Rally {rally_id} Statistics:",
        "match_stats_header": "Match Statistics",
        "upper_player_label": "Upper Court Player:",
        "lower_player_label": "Lower Court Player:",
        "avg_speed_label": "Average Speed",
        "max_speed_label": "Maximum Speed",
        "distance_moved_label": "Distance Moved",
        "total_distance_label": "Total Distance",
        "avg_distance_per_rally_label": "Average Distance per Rally",
        "speed_unit": "m/s",
        "distance_unit": "m",
        "upper_rally_legend": "Upper Court Rally {rally}",
        "upper_players_legend": "Upper Court Players",
        "lower_rally_legend": "Lower Court Rally {rally}",
        "lower_players_legend": "Lower Court Players",
        "viz_error": "Error during visualization",
        "analyzing_data": "Analyzing player position data",
        "analysis_complete": "Player position analysis complete, visualizations saved to",
        "analysis_failed": "Player position analysis failed",
        "select_file_prompt": "Please select detections.jsonl file...",
        "file_dialog_title": "Select Player Position Detection File",
        "jsonl_filetype_label": "JSONL files",
        "all_files_label": "All files",
        "no_file_selected": "No file selected, exiting program",
        "test_success": "Visualization test completed successfully",
        "test_failed": "Visualization test failed",
...
class PlayerPositionVisualizer:
    ...
    def __init__(self, detections_path, output_dir=None, court_width=6.1, court_length=13.4, fps=30, language='zh'):
        """
        ...
        Args:
            ...
            language: UI language for chart titles/labels/stats text ('zh' or 'en')
        """
    ...
    def _generate_heatmap(self, upper_df, lower_df, filename):
        ...
        print(f"{self.strings['heatmap_saved']}: {save_path}")
    ...
    def _add_stats_to_plot(self, rally_id=None):
        ...
        if rally_id is not None:
            ...
            info_text = s["rally_stats_header"].format(rally_id=rally_id) + "\n"
            info_text += "---------------\n"

            # 上场球员统计
            if 'upper' in stats:
                upper_stats = stats['upper']
                info_text += f"{s['upper_player_label']}\n"
                info_text += f"  {s['avg_speed_label']}: {upper_stats['avg_speed']:.2f} {s['speed_unit']}\n"
                info_text += f"  {s['max_speed_label']}: {upper_stats['max_speed']:.2f} {s['speed_unit']}\n"
                info_text += f"  {s['distance_moved_label']}: {upper_stats['total_distance']:.2f} {s['distance_unit']}\n"

            # 下场球员统计
            if 'lower' in stats:
                lower_stats = stats['lower']
                info_text += f"\n{s['lower_player_label']}\n"
                info_text += f"  {s['avg_speed_label']}: {lower_stats['avg_speed']:.2f} {s['speed_unit']}\n"
                info_text += f"  {s['max_speed_label']}: {lower_stats['max_speed']:.2f} {s['speed_unit']}\n"
                info_text += f"  {s['distance_moved_label']}: {lower_stats['total_distance']:.2f} {s['distance_unit']}\n"
        else:
            # 整场比赛的统计信息
            info_text = f"{s['match_stats_header']}\n"
            info_text += "=================\n"
            ...
            if upper_distances:
                ...
                info_text += f"  {s['total_distance_label']}: {total_distance:.2f} {s['distance_unit']}\n"
                info_text += f"  {s['avg_distance_per_rally_label']}: {total_distance/len(upper_distances):.2f} {s['distance_unit']}\n"
            if upper_avg_speeds:
                avg_speed = sum(upper_avg_speeds) / len(upper_avg_speeds)
                info_text += f"  {s['avg_speed_label']}: {avg_speed:.2f} {s['speed_unit']}\n"
            if upper_speeds:
                info_text += f"  {s['max_speed_label']}: {max(upper_speeds):.2f} {s['speed_unit']}\n"
            ...
            if lower_distances:
                ...
                info_text += f"  {s['total_distance_label']}: {total_distance:.2f} {s['distance_unit']}\n"
                info_text += f"  {s['avg_distance_per_rally_label']}: {total_distance/len(lower_distances):.2f} {s['distance_unit']}\n"
            if lower_avg_speeds:
                avg_speed = sum(lower_avg_speeds) / len(lower_avg_speeds)
                info_text += f"  {s['avg_speed_label']}: {avg_speed:.2f} {s['speed_unit']}\n"
            if lower_speeds:
                info_text += f"  {s['max_speed_label']}: {max(lower_speeds):.2f} {s['speed_unit']}\n"

        # 在图表中心右侧添加文本框，适合深色背景
        font_kwargs = {'family': 'SimHei'} if self.language == 'zh' else {'family': 'monospace'}
    ...
    def _generate_scatter_plot(self, upper_df, lower_df, filename):
            ...
            if 'rally_id' in upper_df.columns:
                ...
                for rally_id, rally_data in upper_df.groupby('rally_id'):
                    plt.scatter(
                        ...
                        label=s['upper_rally_legend'].format(rally=int(rally_id)) if rally_id == upper_df['rally_id'].iloc[0] else "_nolegend_"
                    )
            else:
                plt.scatter(
                    upper_df['court_x'],
            ...
            if 'rally_id' in lower_df.columns:
                ...
                for rally_id, rally_data in lower_df.groupby('rally_id'):
                    plt.scatter(
                        ...
                        label=s['lower_rally_legend'].format(rally=int(rally_id)) if rally_id == lower_df['rally_id'].iloc[0] else "_nolegend_"
                    )
            else:
                plt.scatter(
                    lower_df['court_x'],
        ...
        print(f"{self.strings['scatter_saved']}: {save_path}")
    ...
    def visualize(self):
        ...
        try:
            ...
            print(f"{self.strings['viz_error']}: {e}")
            return False


def analyze_player_positions(detections_path, output_dir=None, fps=30, language='zh'):
    """
    ...
    Args:
        detections_path: Path to detections.jsonl containing player position data
        output_dir: Output directory, defaults to visualizations subdirectory in the detection file's directory
        fps: Video frame rate, used for speed calculation
        language: UI language for chart titles/labels/stats text ('zh' or 'en')

    Returns:
        bool: Whether processing was successful
    """
    strings = _LANG.get(language, _LANG['zh'])
    print(f"\n{strings['analyzing_data']}: {detections_path}")

    # Create visualizer
    visualizer = PlayerPositionVisualizer(detections_path, output_dir, fps=fps, language=language)

    # Execute visualization
    success = visualizer.visualize()

    if success:
        print(f"{strings['analysis_complete']}: {visualizer.output_dir}")
    else:
        print(strings['analysis_failed'])
...
if __name__ == "__main__":
    ...
    _language = os.environ.get("PLAYER_POSITIONS_LANG", "zh")
    _strings = _LANG.get(_language, _LANG['zh'])

    # Use file dialog to select detection file
    print(_strings["select_file_prompt"])
    ...
    try:
        ...
        file_path = filedialog.askopenfilename(
            title=_strings["file_dialog_title"],
            filetypes=[(_strings["jsonl_filetype_label"], "*.jsonl"), (_strings["all_files_label"], "*.*")],
            initialdir=default_dir
        )

        # 如果用户取消选择，则退出
        if not file_path:
            print(_strings["no_file_selected"])
            sys.exit(0)

        # 调用分析函数
        success = analyze_player_positions(file_path, language=_language)

        if success:
            print(f"\n{_strings['test_success']}")
        else:
            print(f"\n{_strings['test_failed']}")

    except Exception as e:
        print(f"\n{_strings['test_error']}: {e}")
